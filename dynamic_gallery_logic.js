...
function initGallery(preloadedSearch) {
  ...
  function createFilters() {
    const groups = ['studio', 'pornstar', 'tag'];
    const sets = Object.fromEntries(groups.map(group => [group, new Set()]));

    galleryData.forEach(item => {
      groups.forEach(group => {
        (Array.isArray(item[group]) ? item[group] : [item[group]]).forEach(value => sets[group].add(value));
      });
    });
  ...
  window.currentPage = page;
    ...
    grid.innerHTML = filteredData.slice(start, end).map((item, index) => {
      ...
      const searchLinks = item.pornstar.map(name =>
        `<a href="${base}?search=${encodeURIComponent(name)}" target="_blank">${name}</a>`
      ).join(', ');

      return `
        <div class="col-md-4 mb-4">
          <div class="card h-100 ${selectedLinks.has(item.magnet_link) ? 'selected-card' : ''}">
            ...
            <div class="card-body d-flex flex-column">
              <div class="d-flex align-items-start gap-2 mb-2">
                <input class="form-check-input select-item"
                       ...
                       type="checkbox"
                       value="${item.magnet_link}" id="${id}" ${selectedLinks.has(item.magnet_link) ? 'checked' : ''}>
                <label for="${id}" class="fw-semibold text-dark title-clamp" style="cursor: pointer;">
                  ${item.title}
                </label>
              </div>
              <small class="text-muted">${item.studio} | Size: ${item.size}</small>
              <p><strong>Pornstar:</strong> ${searchLinks}</p>
              <button class="btn btn-outline-primary btn-sm mt-2" onclick="copyMagnet(\'${item.magnet_link}\', this)">Copy Magnet Link</button>
            </div>
          </div>
        </div>`;
    }).join('');
...
function applyFilters() {
  ...
  const groups = ['studio', 'pornstar', 'tag'];
  const active = Object.fromEntries(groups.map(group => [group, []]));
  ...
  filteredData = galleryData.filter(item => {
    const match = groups.every(group =>
      active[group].length === 0 ||
      active[group].some(v => {
        const values = Array.isArray(item[group]) ? item[group] : [item[group]];
    ...
    const matchSearch = !searchTerm ||
      ...
      (item.studio && item.studio.toLowerCase().includes(searchTerm)) ||
      (item.tag && item.tag.some(t => t.toLowerCase().includes(searchTerm))) ||
      (item.pornstar && item.pornstar.some(p => p.toLowerCase().includes(searchTerm)));
